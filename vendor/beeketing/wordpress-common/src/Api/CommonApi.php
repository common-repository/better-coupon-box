...
use BeeketingBCBCommon\Data\AppCodes;
use BeeketingBCBCommon\Data\Constant;
use BeeketingBCBCommon\Data\Setting;
use BeeketingBCBCommon\Data\Webhook;
use BeeketingBCBCommon\Libraries\Helper;
use BeeketingBCBCommon\Libraries\QueryHelper;
...
class CommonApi
{
    ...
    public function install_app()
    {
        ...
        if ( isset( $result['hit'], $result['shop_id'] ) && $result['shop_id'] ) { // Install successfully
            ...
            if ( isset( $_COOKIE[$data_cookie_key] ) ) {
    ...
    private function get_page_snippet()
    {
        ...
        if ( Helper::is_woocommerce_active() ) {
            ...
            if ( is_shop() ) {
                ...
                $product_id = null;
                if ( $product ) {
                    $product_id = Helper::is_wc3() ? $product->get_id() : $product->id;
                }
                $data['page']['type'] = 'product';
                $data['page']['id'] = (int)$product_id;
            } elseif ( is_cart() ) {
                $data['page']['type'] = 'cart';
